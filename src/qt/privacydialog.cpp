                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTCH ought to be enough for anybody." - Bill Gates, 2017
    ui->zTCHpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTCHSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTCH->setEnabled(false);
        ui->pushButtonMintzTCH->setToolTip(tr("zTCH is currently disabled due to maintenance."));

        ui->pushButtonSpendzTCH->setEnabled(false);
        ui->pushButtonSpendzTCH->setToolTip(tr("zTCH is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTCHpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTCH_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTCH is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTCH...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTCH in ") +
...
void PrivacyDialog::on_pushButtonSpendzTCH_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTCH is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTCH
        sendzTCH();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTCH
    sendzTCH();
}

void PrivacyDialog::on_pushButtonZTCHControl_clicked()
{
    ZTCHControlDialog* zTCHControl = new ZTCHControlDialog(this);
    zTCHControl->setModel(walletModel);
    zTCHControl->exec();
}

void PrivacyDialog::setZTCHControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTCHSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTCH()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid TheCash Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTCHpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTCH is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TCH </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTCH</b>";
    ...
    // use mints from zTCH selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTCHControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTCHControlDialog::GetSelectedMints();
    }
    ...
    // Clear zTCH selector in case it was used
    ZTCHControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTCH Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " TCH, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTCH Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTCH </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTCH "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTCH "));
    ui->labelzTCHAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTCHSyncStatus->setVisible(fShow);
}
