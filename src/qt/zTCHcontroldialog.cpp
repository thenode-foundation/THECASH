...
#include "ui_zTCHcontroldialog.h"
...
std::list<std::string> ZTCHControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTCHControlDialog::listMints;

ZTCHControlDialog::ZTCHControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTCHControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTCHControlDialog::~ZTCHControlDialog()
{
    delete ui;
}

void ZTCHControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTCHControlDialog::updateList()
{
...
void ZTCHControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTCHControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTCH_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTCHControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTCHControlDialog::GetSelectedMints()
{
...
void ZTCHControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
